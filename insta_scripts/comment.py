...
class Comment:
    ...
    def commentStart(self,cookie_user_id,comment,auth_proxy,ip_port_proxy,db_pigeonid,db_claim,db_deviceid,db_phoneid,db_androidid,db_USER_AGENT,db_checksum,db_authorization,db_mid,db_userid,db_rur,db_waterfallid,db_adid,db_guid,db_mykey):
        if self.userMedia:    
        ...
        try:
            ...
            print("d4")
            statuslog = self.commentstart()
            print(statuslog)
            if statuslog['status'] == "ok":
                print('status ok geldi ......... ')
        ...
        print({'status':statuslogout,'message':self.error_message})
        return {'status':statuslogout,'message':self.error_message}
...
######## yorum #####################
    ...
    def commentstart(self):
        ...
        mentionControl = True
        while mentionControl:
            print("guncel yorum : ",self.comment)
            sss = self.gen_user_breadcrumb(len(self.comment))
            ...
            alfason = {
                ...
                "_uid": self.userid,
                "_uuid": self.guid,
                "nav_chain": "039:feed_timeline:1,8ff:feed_short_url:2,CommentThreadFragment:comments_v2_feed_short_url:3,CommentThreadFragment:comments_v2_feed_short_url:4",
            ...
            link = "media/" + self.media_id + "/comment/"
            ...
            print("çıktı: ", self.json1)
            if self.json1['status'] == 'fail':

                print(self.json1['message'])
                if self.json1['message'] == 'failed to mention':
                    for x in self.json1['non_mentionable_users']:

                        self.comment = self.comment.replace(str('@')+x['username'],'')
                else:
                    break
            else:
                break

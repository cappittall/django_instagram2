...
class LiveLike:
    ...
    def liveLikeStart(self,cookie_user_id,ip_port_proxy,auth_proxy,db_pigeonid,db_claim,db_deviceid,db_phoneid,db_androidid,db_USER_AGENT,db_checksum,db_authorization,db_mid,db_userid,db_rur,db_waterfallid,db_adid,db_guid,db_mykey):
        ...
        try:
            ...
            self.liveid1()
            print("f2")
            live_like_status = self.livelike1()
            if str(live_like_status) == '200':
                  self.live_like_count +=1
                  liveLikeStatus = True
            print("f3")
        ...
        return {'status':liveLikeStatus,'message':self.error_message}
    ...
    def livelike1(self):
        ...
        try:
            ...
            link = "live/" + self.liveid + "/like/"

            status_code = self.SendRequestloginsonrasitoplu2post1a(link, signature18)
    ...
    def SendRequestloginsonrasitoplu2post1a(self, endpoint, post=None):
        ...
        response2 = requests.post('https://b.i.instagram.com/api/v1/' + endpoint, headers = headers,data=post,verify=True,proxies=self.root_proxy, timeout=15)  # proxies=proxy
        print("Status1: ", response2.status_code)

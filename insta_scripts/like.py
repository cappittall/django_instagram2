...
class Like:
    ...
    def likeStart(self, cookie_user_id, auth_proxy, ip_port_proxy, db_pigeonid, db_claim, db_deviceid, db_phoneid,
        ...
        try:
            ...
            print(e)
        print('return data : ',{'status': status, 'message': self.error_message, 'like_start_count': self.like_start_count})
        return {'status': status, 'message': self.error_message, 'like_start_count': self.like_start_count}
    ...
    def like1(self):
        ...
        try:
            ...
            signature18 = "signed_body=SIGNATURE.{}&d=1".format(ddd80)
            # print(signature18)

            link = "media/" + self.media_id + "/like/"

            status = self.SendRequestloginsonrasitoplu2post1a(link, signature18)
        except Exception as e:
            print("cccc: ", e)
            self.changeProxy()
            self.like1()

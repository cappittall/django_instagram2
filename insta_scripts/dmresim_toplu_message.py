...
class DMResimTopluMessage:
    ...
    def dmResimTopluMessageStart(self, cookie_user_id, ip_port_proxy, auth_proxy, db_pigeonid, db_claim, db_deviceid,
    ...
    def DM_resim_gonder(self):


        try:

            url = 'direct_v2/threads/broadcast/media_share/?media_type=photo'
            ...
            picturedmpostid = self.media_id

            data = 'recipient_users=' + ddd81 + '&action=send_item&is_shh_mode=0&send_attribution=feed_contextual_chain&client_context=' + self.timeson12 + '&media_id=' + picturedmpostid + '&device_id=' + self.androidid + '&mutation_token=' + self.timeson12 + '&_uuid=' + self.guid + '&offline_threading_id=' + self.timeson12
